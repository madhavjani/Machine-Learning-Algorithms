...
class NaiveBayes:
    def fit(self, x, y):
        ...
        self.classification = np.unique(y)
        nclassification = len(self.classification)
        self.mean = np.zeros((nclassification, feature_size), dtype=np.float64)
        self.variance = np.zeros((nclassification, feature_size), dtype=np.float64)
        self.prior = np.zeros(nclassification, dtype=np.float64)

        for index, c in enumerate(self.classification):
            class_x = x[y == c]
            self.mean[index, :] = class_x.mean(axis=0)
            self.variance[index, :] = class_x.var(axis=0)
            self.prior[index] = class_x.shape[0] / float(sample_size)
    ...
    def prediction(self, x):
        ...
        for index, c in enumerate(self.classification):
            ...
            posterior = np.sum(np.log(self.frequency(index, x)))
            posterior = prior + posterior
            posti.append(posterior)

        return self.classification[np.argmax(posti)]

    def frequency(self, class_index, x):
...
### BREAST CANCER DATA ###
    ...
    if dataset == "breast_cancer":
        ...
        print("NaiveBayes standard deviation for", sys.argv[1], standard_deviation)
