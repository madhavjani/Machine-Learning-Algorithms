...
class KNN:
    ...
    def predict(self, x):
        y_pred = [self.predict_val(x) for x in x]
        return np.array(y_pred)

    def predict_val(self, x):
        dist = [eud(x, x_train) for x_train in self.x_train]
        index = np.argsort(dist)[: self.k]
        neighbour_label = [self.y_train[i] for i in index]
        common = Counter(neighbour_label).most_common(1)
        return common[0][0]
...
if __name__ == "__main__":
    ...
    if dataset == 'breast_cancer':
        ...
        print("KNN standard deviation for", sys.argv[1], standard_deviation)
        ...
        df = pd.read_csv("letter-recognition.data",
                         names=["lettr", "x-box", "y-box", "width", "high", "onpix", "x-bar", "y-bar", "x2bar", "y2bar",
